...
class DNN(object):
    ...
    def build_sgd_functions(self, datasets, batch_size, 
        ...
        return train_fn, valid_fn, valid_score, valid_train_score
...
def test_sda(
    ...
    os.chdir("./results_nopre0.08")
    fp = open(dataset + '.res', 'w')
    ...
    train_fn, vl_fn, valid_fn, valid_train_fn = dnn.build_sgd_functions(
    ...
    while epoch < n_epochs:
        ...
        if epoch % 1 == 0:
            ...
            this_valid_corrcoef = np.corrcoef(valid_preds, 
                                                    valid_dataset)[0][1]
            fp.write('%d %.3f %.3f %.3f %.3f\n' %(epoch, tr_cost.mean(), vl_cost.mean(), this_train_corrcoef, this_valid_corrcoef))
            if best_corrcoef < this_valid_corrcoef:
                best_corrcoef = this_valid_corrcoef
                best_epoch = epoch
                best_valid_pred = valid_preds
...
if __name__ == '__main__':
    ...
    test_sda(
            ...
            pretrain_epochs = 0,
            learning_way = 'adagrad', # normal/adagrad/adadelta
            learning_rate = 0.08,
            n_epochs = 500,
            dataset = argvs[1],
            batch_size = 4,
            layer_sizes = [1656, 800, 400, 1],
